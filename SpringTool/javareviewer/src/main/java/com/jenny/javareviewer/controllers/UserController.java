...
//		userService.update(user);
	...
	public String joinGuild(@ModelAttribute("User")User user, BindingResult result){
		...
		// THIS IS USER ID
		String userId= (String) result.getFieldValue("firstName");
		Long user_id= Long.parseLong(userId.trim());
		User itsUser= userService.findById(user_id);
		
		// THIS IS GUILD ID
		String guildId= (String) result.getFieldValue("lastName");
		Long guild_id = Long.parseLong(guildId.trim());
		Guild itsGuild= guildService.findById(guild_id);
		
		List<User> list = itsGuild.getUsers();
		list.add(itsUser);		
		itsGuild.setUsers(list);
		guildService.create(itsGuild);
